<template>
  ...
  >
    ...
    <div
      v-if="dataSourse.length != 0 || loading"
    ...
    >
      ...
      <!-- <div class="filterMenu" v-if="!isMobile">
        <div class="box">
          ...
          >
            ...
            <DropdownMenu slot="list">
              <CheckboxGroup v-model="filters.transactionType">
                <Checkbox label="Build"> Build </Checkbox>
                <Checkbox label="Burn"> Burn </Checkbox>
                <Checkbox label="Claim"> Claim </Checkbox>
                <Checkbox label="Transfer"> Transfer </Checkbox>
          ...
          <Dropdown
            ...
            :class="{
              hasFilter: filters.amountRange.from || filters.amountRange.to,
            }"
          >
            <div
              class="amountBtn"
              v-if="filters.amountRange.from || filters.amountRange.to"
            >
              From
              {{ filters.amountRange.from ? filters.amountRange.from : 0 }}
              to
              {{ filters.amountRange.to ? filters.amountRange.to : 0 }}
            </div>
      ...
      <div class="customTable">
        <div class="tabelHeader">
            <div class="th sort no">NO</div>
            <div class="th sort txid">txid</div>
            <div class="th sort address">address</div>
            <div class="th sort detail">detail</div>
            <div class="th sort txType">txType</div>
            <div class="th sort chainId">chainId</div>
            <div class="th sort time"> time</div>
        </div>

        <div v-if="dataSourse.length" class="hasData">
          <div class="tableBody">
            <div class="tr" v-for="(row, index) in dataSourse" :key="index">
                <div class="td sort no">{{ row.index }}</div>
                <div class="td sort txid">{{ row.txid }}</div>
                 <div class="td sort address">{{ row.address }}</div>
                <div class="td sort detail">{{ row.detail }}</div>
                <div class="td sort txType">{{ row.txType }}</div>
                <div class="td sort chainId">{{ row.chainId }}</div>
                <div class="td sort time">{{ row.time }}</div>
            </div>
          </div>
        </div>
        <div v-else class="notData">No Data</div>
    ...
    >
      ...
      <div class="customTable">
        <div class="tabelHeader">
          ...
          <div class="th sort date">Date</div>
          <template v-if="!isMobile">
            <div class="th sort type">Type</div>
            <div class="th sort amount">Amount</div>
          </template>
          <template v-if="isMobile">
            <div class="th sort typeAmount">Type/Amount</div>
          </template>
        ...
        <div v-if="tableData.length" class="hasData">
          <div class="tableBody">
            <div class="tr" v-for="(row, index) in tableData" :key="index">
              <div class="td chain">
                <template v-if="isEthereumNetwork(row.networkId)">
                  <img src="@/static/ETH.svg" />
                </template>
                <template v-if="isBinanceNetwork(row.networkId)">
    ...
    >
      ...
      <div class="filterBox">
        ...
        >
          <div class="typeBtn" v-if="mobileFilters.transactionType.length == 0">
            <span>All Types</span><Icon type="ios-arrow-down"></Icon>
          </div>
          <div
            class="typeBtnSelected"
        ...
        <Dropdown
          ...
          :class="{
            hasFilter:
              mobileFilters.amountRange.from || mobileFilters.amountRange.to,
          }"
        >
          <div
            class="amountBtn"
          ...
          >
            ...
            to
            {{
              mobileFilters.amountRange.to ? mobileFilters.amountRange.to : 0
            }}
          </div>
          <div class="amountBtnSelected" v-else>
            <span>All Amount</span><Icon type="ios-arrow-down"></Icon>
...
export default {
  ...
  created() {
    //订阅组件改变事件
    this.$pub.subscribe("transactionModalChange", async (msg, params) => {
    ...
    currentPageData() {
      ...
      this.currentPageData.map(function (item, index, ary) {
        ...
        if (
          ...
          let rewardssUSD = "";
          let rewardsLina = "";
          if (item.rewardssUSD != undefined || item.rewardssUSD != 0) {
  ...
  computed: {
    ...
    filterTransactionHistoryData() {
      ...
      var filterTransactionHistoryData = this.transactionHistoryData.filter(
        function (record) {
          var { chainType, transactionType, dateRange, amountRange } =
            that.filters;
          ...
          if (
            ...
            (record.timestamp < Date.parse(dateRange[0]) ||
              record.timestamp > Date.parse(dateRange[1]) + 86400000)
          ) {
            return false;
          }
  ...
  methods: {
    ...
    //xxl ##01 get data from 
    // 获取表格数据
    async getData(){
        const filterAccount = (v) => `${v.slice(0, 5)}...${v.slice(v.length - 5, v.length - 1)}`
        ...
        const data = await api.getTransaction(this.$store.state?.wallet?.address);
        console.log("xxl getData12");
        console.log(data.data);
        // data.data = [
        //   {
        //     txid: '0x83445F2b35387DF967Fc3f3D4ff84EFd4c3F7987',
        //     txFrom: '0x83445F2b35387DF967Fc3f3D4ff84EFd4c3F7987',
        //     txValue: '1shmd=>5usdt',
        //     txType: 0,
        //     txChain: '42',
        //     time: '2020-52-59 06:05:26'
        //   }
        // ]
        ...
        // const mock = [
        //     { txid: filterAccount('0x83445F2b35387DF967Fc3f3D4ff84EFd4c3F7987'), from: '1', to: '1', value: 2323.111111111, txType: 'txType', chain: 'oky', times: '1999-03-01' },
        //     { txid: '12345689', from: '1', to: '1', value: 2323.111111111, txType: 'txType', chain: 'oky', times: '1999-03-01' },
        //     { txid: '12345689', from: '1', to: '1', value: 2323.111111111, txType: 'txType', chain: 'oky', times: '1999-03-01' }
        // ].map((v, i) => ({
        //     ...v,
        //     index: i + 1
        ...
        for(var i = 0 ;i < data.data.length; i ++ ){
          var each = {
            "index":i + 1,
            "txid":filterAccount(data.data[i].txid),
            "address":filterAccount(data.data[i].txFrom),
            "detail":data.data[i].txValue,
            "txType":mapTxType[data.data[i].txType],
            "chainId":data.data[i].txChain,
        ...
        console.log("xxl mock ...");
        console.log(mock);

        this.dataSourse = mock;
    },
    ...
    async fetchTransactionHistoryClick() {
      ...
      this.transactionHistoryData = this.transactionHistoryData.sort(function (
        record1,
        record2
      ) {
