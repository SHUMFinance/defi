...
/**
 * @title ShumRewardLocker
 *
 * @dev A contract for locking Shum rewards. The current version only supports adding rewards.
 * Reward claiming will be added in a later iteration.
 */
contract ShumRewardLocker is IShumRewardLocker, ShumAdminUpgradeable {
    ...
    modifier onlyLockRewardRole() {
        require(accessCtrl.hasRole(ROLE_LOCK_REWARD, msg.sender), "ShumRewardLocker: not LOCK_REWARD role");
        _;
    }

    modifier onlyMoveRewardRole() {
        require(accessCtrl.hasRole(ROLE_MOVE_REWARD, msg.sender), "ShumRewardLocker: not MOVE_REWARD role");
        _;
    }
    ...
    function migrateRewards(
        ...
        require(users.length > 0, "ShumRewardLocker: empty array");
        require(users.length == amounts.length && amounts.length == unlockTimes.length, "ShumRewardLocker: length mismatch");
    ...
    function _addReward(
        ...
        require(amount > 0, "ShumRewardLocker: zero amount");

        uint216 trimmedAmount = uint216(amount);
        uint40 trimmedUnlockTime = uint40(unlockTime);
        require(uint256(trimmedAmount) == amount, "ShumRewardLocker: reward amount overflow");
        require(uint256(trimmedUnlockTime) == unlockTime, "ShumRewardLocker: unlock time overflow");
    ...
    function _moveRewardProRata(
        ...
        require(totalAmount > 0 && totalAmount <= lockedAmountByAddresses[from], "ShumRewardLocker: amount out of range");
        ...
        require(amount1Left == 0 && amount2Left == 0, "ShumRewardLocker: amount not filled with all entries");
    }
