 ...
 */
contract ShumErc20Bridge is ShumAdminUpgradeable {
    ...
    function __ShumErc20Bridge_init(address _relayer, address _admin) public initializer {
    ...
    function addToken(
        ...
        require(tokenInfos[tokenKey].tokenAddress == address(0), "ShumErc20Bridge: token already exists");
        require(tokenAddress != address(0), "ShumErc20Bridge: zero address");
        require(
            lockType == TOKEN_LOCK_TYPE_TRANSFER || lockType == TOKEN_LOCK_TYPE_MINT_BURN,
            "ShumErc20Bridge: unknown token lock type"
        );
    ...
    function removeToken(bytes32 tokenKey) external onlyAdmin {
        require(tokenInfos[tokenKey].tokenAddress != address(0), "ShumErc20Bridge: token does not exists");
    ...
    function addChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyAdmin {
        require(!tokenSupportedOnChain[tokenKey][chainId], "ShumErc20Bridge: already supported");
    ...
    function dropChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyAdmin {
        require(tokenSupportedOnChain[tokenKey][chainId], "ShumErc20Bridge: not supported");
    ...
    function deposit(
        ...
        require(destChainId != currentChainId, "ShumErc20Bridge: dest must be different from src");
        require(isTokenSupportedOnChain(token, destChainId), "ShumErc20Bridge: token not supported on chain");
        ...
        if (tokenInfo.lockType == TOKEN_LOCK_TYPE_TRANSFER) {
            ...
            require(false, "ShumErc20Bridge: unknown token lock type");
        }
    ...
    function _setRelayer(address _relayer) private {
        require(_relayer != address(0), "ShumErc20Bridge: zero address");
        require(_relayer != relayer, "ShumErc20Bridge: relayer not changed");
    ...
    function safeTransfer(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "ShumErc20Bridge: transfer failed");
    }
    ...
    function safeTransferFrom(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "ShumErc20Bridge: transfer from failed");
    }
}
