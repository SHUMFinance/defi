...
/**
 * @title ShumVaultFixedRewardPool
 *
 ...
 */
contract ShumVaultFixedRewardPool is OwnableUpgradeable {
    ...
    modifier onlyStarted() {
        require(block.timestamp >= startTime, "ShumVaultFixedRewardPool: pool not started");
        _;
    }
    ...
    function __ShumVaultFixedRewardPool_init(
        uint256 _startTime,
        ...
        require(_startTime > block.timestamp, "ShumVaultFixedRewardPool: invalid start time");
        require(_rewardPerSecond > 0, "ShumVaultFixedRewardPool: zero reward");
        require(_stakeToken != address(0), "ShumVaultFixedRewardPool: zero address");
        require(_rewardToken != address(0), "ShumVaultFixedRewardPool: zero address");
    ...
    function claimReward() external {
        ...
        require(rewardToClaim > 0, "ShumVaultFixedRewardPool: no reward to claim");
    ...
    function _stake(address user, uint256 amount) private {
        require(amount > 0, "ShumVaultFixedRewardPool: cannot stake zero amount");
    ...
    function _unstake(address user, uint256 amount) private {
        require(amount > 0, "ShumVaultFixedRewardPool: cannot unstake zero amount");
