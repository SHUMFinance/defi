...
contract ShumDebtSystem is ShumAdminUpgradeable, ShumAddressCache {
    ...
    // -------------------------------------------------------
    function __ShumDebtSystem_init(address _admin) public initializer {
    ...
    function updateAddressCache(IShumAddressStorage _addressStorage) public override onlyAdmin {
        ...
        assetSys = IShumAssetSystem(_addressStorage.getAddressWithRequire("ShumAssetSystem", "ShumAssetSystem address not valid"));

        emit CachedAddressUpdated("ShumAccessControl", address(accessCtrl));
        emit CachedAddressUpdated("ShumAssetSystem", address(assetSys));
    }
    ...
    function _pushDebtFactor(uint256 _factor) private {
        ...
        if (lastDeletTo < lastCloseAt) {
            // safe check
            uint256 deShumum = lastCloseAt - lastDeletTo;
            deShumum = (deShumum > MAX_DEL_PER_TIME) ? MAX_DEL_PER_TIME : deShumum; // not delete all in one call, for saving someone fee.
            for (uint256 i = lastDeletTo; i < deShumum; i++) {
                delete lastDebtFactors[i];
            }
            lastDeletTo = lastDeletTo.add(deShumum);
        }
    }
