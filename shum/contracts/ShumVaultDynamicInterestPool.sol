...
contract ShumVaultDynamicInterestPool is OwnableUpgradeable {
    ...
    function __ShumVaultDynamicInterestPool_init(
        ...
        require(_firstPeriodStartTime > block.timestamp, "ShumVaultDynamicInterestPool: invalid time range");
        require(_periodDuration > 0, "ShumVaultDynamicInterestPool: zero duration");
        require(_totalSubscriptionLimit > 0, "ShumVaultDynamicInterestPool: zero subscription limit");
        require(_stakeToken != address(0), "ShumVaultDynamicInterestPool: zero address");
        require(_interestToken != address(0), "ShumVaultDynamicInterestPool: zero address");
    ...
    function withdrawInterests(uint256 fromPeriodId, uint256 toPeriodId) external {
        require(fromPeriodId > 0 && toPeriodId >= fromPeriodId, "ShumVaultDynamicInterestPool: invalid period range");
    ...
    function setInterestRate(uint256 periodId, uint256 interestRate) external onlyOwner {
        require(periodId > 0, "ShumVaultDynamicInterestPool: invalid period id");
        require(interestRate > 0, "ShumVaultDynamicInterestPool: zero rate");

        require(interestRates[periodId] == 0, "ShumVaultDynamicInterestPool: rate already set");
    ...
    function _adjustSubscription(address user, int256 amount) private {
        require(amount != 0, "ShumVaultDynamicInterestPool: zero amount");
        ...
        if (currentUserData.lastInterestWithdrawalPeriodId == currentPeriodId) {
            ...
            require(newAmountSigned >= 0, "ShumVaultDynamicInterestPool: insufficient amount");

            uint256 newAmountUnsigned = newAmountSigned.toUint256();
            require(newAmountUnsigned <= userSubscriptionLimit, "ShumVaultDynamicInterestPool: user oversubscribed");
            ...
            require(
                ...
                "ShumVaultDynamicInterestPool: amount change pending"
            );
            ...
            require(newOverallAmountSigned >= 0, "ShumVaultDynamicInterestPool: insufficient amount");

            require(
                newOverallAmountSigned.toUint256() <= userSubscriptionLimit,
                "ShumVaultDynamicInterestPool: user oversubscribed"
            );
        ...
        require(totalSubscribedAmount <= totalSubscriptionLimit, "ShumVaultDynamicInterestPool: total oversubscribed");
        ...
        if (amount > 0) {
            ...
            require(
                ...
                "ShumVaultDynamicInterestPool: withdraw pending refund first"
            );
    ...
    function _withdrawPrincipal(address user) private {
        ...
        require(refundingAmount > 0, "ShumVaultDynamicInterestPool: no refund pending");
        require(currentPeriodId >= refundPeriodId, "ShumVaultDynamicInterestPool: refund still pending");
    ...
    function _withdrawInterest(address user, uint256 periodId) private {
        ...
        require(periodId < currentPeriodId, "ShumVaultDynamicInterestPool: period not ended");
        require(
            periodId == currentUserData.lastInterestWithdrawalPeriodId + 1,
            "ShumVaultDynamicInterestPool: invalid period id"
        );
        ...
        require(
            ...
            "ShumVaultDynamicInterestPool: unexpected pending changes"
        );
        ...
        uint256 periodInterestRate = interestRates[periodId];
        require(periodInterestRate > 0, "ShumVaultDynamicInterestPool: interest rate not set");
