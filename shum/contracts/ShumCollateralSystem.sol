...
contract ShumCollateralSystem is ShumAdminUpgradeable, PausableUpgradeable, ShumAddressCache {
    ...
    bytes32 public constant Currency_SHUM = "SHUM";
    ...
    modifier onlyLiquidation() {
        require(msg.sender == liquidation, "ShumCollateralSystem: not liquidation");
        _;
    }

    modifier onlyRewardLocker() {
        require(msg.sender == address(mRewardLocker), "ShumCollateralSystem: not reward locker");
        _;
    }
    ...
    // -------------------------------------------------------
    function __ShumCollateralSystem_init(address _admin) public initializer {
    ...
    function updateAddressCache(IShumAddressStorage _addressStorage) public override onlyAdmin {
        ...
        mRewardLocker = IShumRewardLocker(
            _addressStorage.getAddressWithRequire("ShumRewardLocker", "ShumRewardLocker address not valid")
        );
        buildBurnSystem = IShumBuildBurnSystem(
            _addressStorage.getAddressWithRequire("ShumBuildBurnSystem", "ShumBuildBurnSystem address not valid")
        );
        ...
        emit CachedAddressUpdated("ShumRewardLocker", address(mRewardLocker));
        emit CachedAddressUpdated("ShumBuildBurnSystem", address(buildBurnSystem));
    ...
    function stakeAndBuild(
        ...
        require(stakeAmount > 0 || buildAmount > 0, "ShumCollateralSystem: zero amount");
    ...
    function stakeAndBuildMax(bytes32 stakeCurrency, uint256 stakeAmount) external whenNotPaused {
        require(stakeAmount > 0, "ShumCollateralSystem: zero amount");
    ...
    function burnAndUnstake(
        ...
        require(burnAmount > 0 || unstakeAmount > 0, "ShumCollateralSystem: zero amount");
    ...
    function burnAndUnstakeMax(uint256 burnAmount, bytes32 unstakeCurrency) external whenNotPaused {
        require(burnAmount > 0, "ShumCollateralSystem: zero amount");
    ...
    function _Redeem(
        ...
        emit RedeemCollateral(user, Currency_SHUM, _amount, userCollateralData[user][Currency_SHUM].collateral);
    }
    ...
    function collateralFromUnlockReward(
        ...
        require(user != address(0), "ShumCollateralSystem: User address cannot be zero");
        require(amount > 0, "ShumCollateralSystem: Collateral amount must be > 0");

        TokenInfo storage tokeninfo = tokenInfos[currency];
        require(tokeninfo.tokenAddr != address(0), "ShumCollateralSystem: Invalid token symbol");
