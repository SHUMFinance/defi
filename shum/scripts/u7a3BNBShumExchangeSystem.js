...
const main = async () => {
    ...
    await shumConfig.connect(admin).setUint(
      ethers.utils.formatBytes32String("sBNB"), // key
    ...
    try{
        ...
        let sBNBTokenAddress = await readConfig("15config","sBNB");
        ...
        console.log(sBNBTokenAddress);
        //4.0 set sBNB
        let sBNBToken = await ShumAssetUpgradeable.connect(admin).attach(
          sBNBTokenAddress
        );
        ...
        let beforesBNBBal = await sBNBToken.balanceOf(alice.address)
        console.log(beforesBNBBal);
        ///---------------------------------------------------------------------
        ...
        let tx = await shumExchangeSystem.connect(alice).exchange(
          ...
          ethers.utils.formatBytes32String("sBNB"),   // destKey
          {gasLimit: args.gasLimit}
        ...
        sBNBBala = await sBNBToken.balanceOf(alice.address)
        console.log(sBNBBala);
        ///---------------------------------------------------------------------
