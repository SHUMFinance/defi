...
const main = async () => {
    ...
    await writeConfig("15config","15config","sXLMChainlink",chainlinkAggregator.address);
    await sleep(2000);
    ...
    // Set token "sXLM" to use Chainlink
    await shumOracleRouter.connect(admin).addChainlinkOracle(
        ethers.utils.formatBytes32String("sXLM"), // currencyKey
    ...
    tx = await chainlinkAggregator.setLatestRoundData(
      1, // newRoundId
      expandToNDecimals(2.1276, 18), // newAnswer
      100, // newStartedAt
      200, // newUpdatedAt
