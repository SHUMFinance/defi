...
const main = async () => {
    ...
    await writeConfig("15config","15config","XML2Chainlink",chainlinkAggregator.address);
    await sleep(2000);
    ...
    // Set token "sYFI" to use Chainlink
    await shumOracleRouter.connect(admin).addChainlinkOracle(
        ethers.utils.formatBytes32String("sYFI"), // currencyKey
    ...
    tx = await chainlinkAggregator.setLatestRoundData(
      1, // newRoundId
      expandToNDecimals(35443.4100, 18), // newAnswer
      100, // newStartedAt
      200, // newUpdatedAt
