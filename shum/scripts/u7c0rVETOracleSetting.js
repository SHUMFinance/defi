...
const main = async () => {
    ...
    await writeConfig("15config","15config","VETChainlink",chainlinkAggregator.address);
    await sleep(2000);
    ...
    // Set token "sVET" to use Chainlink
    await shumOracleRouter.connect(admin).addChainlinkOracle(
        ethers.utils.formatBytes32String("sVET"), // currencyKey
    ...
    tx = await chainlinkAggregator.setLatestRoundData(
      1, // newRoundId
      expandToNDecimals(0.1274, 18), // newAnswer
      100, // newStartedAt
      200, // newUpdatedAt
